...
- [x] NN 輸出 5 → 8 維：新增劑量參數（工作容量／恢復力／進步速率），規則引擎標籤＋訓練器同步
- [x] 動作庫改結構化次數區間（`rep_lo/rep_hi/unit`）＋休息基準；規劃器依劑量參數選次數子窗、縮放組數與休息
- [x] 每週漸進斜率、減載深度、跨週能力投影（後段週次預先升階，每 block 最多一階）皆由 NN 決定
